import { calculateTargets, dailyBudget } from './nutrition';
import {
  DailyPlan,
  Food,
  FoodCategory,
...
function cheapestPrice(food: FoodWithPrices): { storeName: string; pricePer100: number } {
  return food.prices.reduce((best, p) => (p.pricePer100 < best.pricePer100 ? p : best));
}

function isEligible(food: FoodWithPrices, profile: UserProfile): boolean {
  ...
  if (food.prices.length === 0) return false;
  return true;
}
 ...
 */
function pickFood(
  eligible: FoodWithPrices[],
  categories: FoodCategory[],
  macroField: 'proteinPer100' | 'carbsPer100',
  usageCount: Record<string, number>,
): FoodWithPrices | undefined {
  ...
  const ranked = [...candidates].sort((a, b) => {
    ...
    return cheapestPrice(a).pricePer100 - cheapestPrice(b).pricePer100; // near-tie: cheaper wins
  });
  return ranked[0];
}

function buildItem(food: FoodWithPrices, gramsRequested: number): PlannedItem {
  const grams = Math.min(gramsRequested, MAX_ITEM_GRAMS);
  const { storeName, pricePer100 } = cheapestPrice(food);
  const factor = grams / 100;
  return {
    food,
    grams: Math.round(grams),
    cheapestStore: storeName,
    cost: Number((pricePer100 * factor).toFixed(2)),
...
function repairBudget(
  ...
  while (totalCost > budget && iterations < 50) {
    ...
    let bestMealIdx = -1;
    let bestItemIdx = -1;
    let bestReplacement: FoodWithPrices | undefined;

    meals.forEach((meal, mi) => {
      meal.items.forEach((item, ii) => {
        const sameCategory = eligible.filter(
          (f) => f.category === item.food.category && f.id !== item.food.id,
        );
        sameCategory.forEach((alt) => {
          const altPrice = cheapestPrice(alt).pricePer100;
          const currentPrice = cheapestPrice(item.food).pricePer100;
          const saving = (currentPrice - altPrice) * (item.grams / 100);
    ...
    meal.items[bestItemIdx] = buildItem(bestReplacement, oldItem.grams);
...
function solveTwoItemGrams(
  proteinFood: FoodWithPrices,
  carbFood: FoodWithPrices,
...
export function generateDailyPlan(profile: UserProfile, foods: FoodWithPrices[]): DailyPlan {
  ...
  const meals: MealSlot[] = schedule.map(({ name, time }) => {
    ...
    const produceItem = produce.length
      ? buildItem(
          [...produce].sort((a, b) => cheapestPrice(a).pricePer100 - cheapestPrice(b).pricePer100)[0],
          PRODUCE_GRAMS,
        )
      : undefined;

    const proteinFood = pickFood(eligible, PROTEIN_CATEGORIES, 'proteinPer100', usageCount);
    const carbFood = pickFood(eligible, CARB_CATEGORIES, 'carbsPer100', usageCount);
    ...
    if (proteinFood && carbFood) {
      const { proteinGrams, carbGrams } = solveTwoItemGrams(
        proteinFood,
        carbFood,
        remainingProtein,
        remainingCarbs,
      );
      items.push(buildItem(proteinFood, proteinGrams));
      items.push(buildItem(carbFood, carbGrams));
      useFood(proteinFood.id);
      useFood(carbFood.id);
    } else if (proteinFood) {
      items.push(buildItem(proteinFood, remainingProtein / (proteinFood.proteinPer100 / 100 || 1)));
      useFood(proteinFood.id);
    } else if (carbFood) {
      items.push(buildItem(carbFood, remainingCarbs / (carbFood.carbsPer100 / 100 || 1)));
      useFood(carbFood.id);
    }
  ...
  const repairedMeals = repairBudget(meals, eligible, budget);

  const totals = {
    calories: repairedMeals.reduce((s, m) => s + m.calories, 0),
    proteinG: repairedMeals.reduce((s, m) => s + m.proteinG, 0),
    carbsG: repairedMeals.reduce((s, m) => s + m.carbsG, 0),
    fatG: repairedMeals.reduce((s, m) => s + m.fatG, 0),
    cost: Number(repairedMeals.reduce((s, m) => s + m.cost, 0).toFixed(2)),
  };

  return {
    targets,
    budgetForDay: Number(budget.toFixed(2)),
